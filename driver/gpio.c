...
//--
volatile uint16_t button_interrupt;

GPIO_ConfigTypeDef pGPIOConfigButton;
GPIO_ConfigTypeDef pGPIOConfigOutput;
...
void gpio_intr_handler(void){  
  ...
  if ((gpio_status & BIT(PRODUCT_BUTON_PIN))){
    //disable interrupt
    gpio_pin_intr_state_set(PRODUCT_BUTON_PIN, GPIO_PIN_INTR_DISABLE);
    button_interrupt|=1<<PRODUCT_BUTON_PIN;
    printf(" Interrupt GPIO5\n");
    //clear interrupt status
   GPIO_REG_WRITE(GPIO_STATUS_W1TC_ADDRESS, gpio_status & BIT(PRODUCT_BUTON_PIN));
   // restore after debounce 
  }
  if ((gpio_status & BIT(WC_BUTON_PIN))){
    //disable interrupt
    gpio_pin_intr_state_set(WC_BUTON_PIN, GPIO_PIN_INTR_DISABLE);
    button_interrupt|=1<<WC_BUTON_PIN;
    printf(" Interrupt GPIO14\n");
    //clear interrupt status
    GPIO_REG_WRITE(GPIO_STATUS_W1TC_ADDRESS, gpio_status & BIT(WC_BUTON_PIN));  
    // restore after debounce 
  }
  if ((gpio_status & BIT(STOPED_BUTON_PIN))){       
    //disable interrupt
    gpio_pin_intr_state_set(STOPED_BUTON_PIN, GPIO_PIN_INTR_DISABLE);
    button_interrupt|=1<<STOPED_BUTON_PIN;
    printf(" Interrupt GPIO4\n");
    //clear interrupt status
    GPIO_REG_WRITE(GPIO_STATUS_W1TC_ADDRESS, gpio_status & BIT(STOPED_BUTON_PIN));
    // restore after debounce 
  }
}
/*#############################################################
...
void gpio_init(void){
  ...
  pGPIOConfigOutput.GPIO_Pullup   = GPIO_PIN_INTR_DISABLE;
 	pGPIOConfigOutput.GPIO_Mode     = GPIO_Mode_Output;
  pGPIOConfigOutput.GPIO_Pullup   = GPIO_PullUp_DIS;
  
 	pGPIOConfigOutput.GPIO_Pin = BIT(BLUE_LED_PIN);// blue led
	gpio_config(&pGPIOConfigOutput);
  pGPIOConfigOutput.GPIO_Pin = BIT(RED_LED_PIN);//red led
	gpio_config(&pGPIOConfigOutput);
  pGPIOConfigOutput.GPIO_Pin = BIT(BUZZER_PIN); //buzzer
	gpio_config(&pGPIOConfigOutput);
 
	GPIO_OUTPUT_SET(BLUE_LED_PIN, LED_OFF); //blue led 
  GPIO_OUTPUT_SET(RED_LED_PIN, LED_OFF); //red led 
  GPIO_OUTPUT_SET(BUZZER_PIN, BUZZER_OFF);  //buzzer
 
  pGPIOConfigButton.GPIO_IntrType = GPIO_PIN_INTR_POSEDGE;
	pGPIOConfigButton.GPIO_Mode     = GPIO_Mode_Input;
	pGPIOConfigButton.GPIO_Pullup   = GPIO_PullUp_EN;

	pGPIOConfigButton.GPIO_Pin = BIT(PRODUCT_BUTON_PIN);//Product
	gpio_config(&pGPIOConfigButton);
 	pGPIOConfigButton.GPIO_Pin = BIT(WC_BUTON_PIN); //WC
 	gpio_config(&pGPIOConfigButton);
 	pGPIOConfigButton.GPIO_Pin = BIT(STOPED_BUTON_PIN); //Stoped
  gpio_config(&pGPIOConfigButton);
